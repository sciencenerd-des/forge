...
pub fn save_profile(role: &str, base_url: &str, model: &str, api_key: &str, auth_mode: &str) -> Result<PathBuf> {
    ...
    } else { json!({"version": 1, "profiles": {}}) };
    if document.get("version") != Some(&json!(1)) { return Err(anyhow!("providers.json version must be 1")); }
    document["profiles"][role] = json!({"base_url": base_url, "model": model, "api_key": api_key, "auth_mode": auth_mode});
...
mod tests {
    ...
    fn writes_shared_provider_shape_with_restrictive_permissions() {
        ...
        unsafe { env::set_var("FORGE_HOME", directory.path()); }
        let path = save_profile("executor", "http://localhost:1234/v1", "model", "secret", "api_key").unwrap();
        let value: Value = serde_json::from_slice(&fs::read(path.clone()).unwrap()).unwrap();
        assert_eq!(value["version"], 1);
        assert_eq!(value["profiles"]["executor"]["model"], "model");
        #[cfg(unix)] assert_eq!(fs::metadata(path).unwrap().permissions().mode() & 0o777, 0o600);
    }
}
